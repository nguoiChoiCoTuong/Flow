...
async function updateWeather(city) {
  ...
  try {
    ...
    const chartDays = forecastData.time || [];
    const normalizedAll = forecastData.meteoData ? normalizeForecast(forecastData.meteoData) : {};
    const rainProbs = chartDays.map(day => {
      const d = normalizedAll[day] || {};
      return d.maxRainProb != null ? Math.round(d.maxRainProb) : 0;
    });

    console.log("Chart data:", { chartDays, rainProbs });

    if (chartDays.length === 0 || rainProbs.length === 0) {
      console.warn("No data available for rain chart");
      const chartContainer = document.getElementById("rainChart").parentElement;
